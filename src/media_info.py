...
class Info():
    ...
    def video_info(self, stream):
        data = {
            ...
            'is_avc': 'avc' in stream.get('codec_name', '').lower(),
            'status': True
        }
        return data
        

    def audio_info(self, stream, path=False):
    ...
    def subtitle_info(self, stream, path=False):
        data = {
            ...
            'is_bitmap': stream.get('codec_name', '').lower() in ['dvd_subtitle', 'hdmv_pgs_subtitle', 'xsub'],
            'is_text': stream.get('codec_name', '').lower() in ['subrip', 'ass', 'ssa', 'mov_text', 'webvtt'],
            'status': True,
            'path': path
        }
        return data
